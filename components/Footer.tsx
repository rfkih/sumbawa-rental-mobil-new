    ...
    <footer id="kontak" className="bg-cream border-t border-line-soft">
      <div className="max-w-[1440px] mx-auto px-6 sm:px-10 lg:px-16 py-16 lg:py-20">
        ...
        <div className="relative bg-ink text-white rounded-3xl p-6 sm:p-10 lg:p-14 overflow-hidden mb-14">
          ...
          <div className="relative grid lg:grid-cols-[1.2fr_1fr] gap-10 lg:gap-16 items-start">
            {/* Left: headline */}
            <div>
              <span className="eyebrow-new mb-4" style={{ color: "#7B9FFF" }}>
                {f.eyebrow}
              </span>
              <h3 className="text-3xl sm:text-4xl font-extrabold tracking-tight mt-4 mb-3">
                {f.title}{" "}
                <span className="text-brand">{f.titleHighlight}</span>
              </h3>
            ...
            <div className="flex flex-col gap-3">
              ...
              >
                ...
                <div>
                  <p className="text-xs font-semibold text-white/70 mb-0.5">WhatsApp</p>
                  <p className="font-bold text-white">{PHONE}</p>
                </div>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="ml-auto opacity-60">
              ...
              >
                ...
                <div>
                  <p className="text-xs font-semibold text-white/60 mb-0.5">Email</p>
              ...
              <div className="flex items-center gap-4 bg-white/10 rounded-2xl px-5 py-4 border border-white/10">
                ...
                <div>
                  <p className="text-xs font-semibold text-white/60 mb-0.5">{f.officeLabel}</p>
                  <p className="font-bold text-white text-sm leading-snug">{f.officeValue}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="grid lg:grid-cols-[1fr_1.2fr] gap-12 lg:gap-16">
          ...
          <div className="flex flex-col gap-6">
            ...
            <p className="text-sm text-muted leading-relaxed max-w-xs">{f.about}</p>

            <div className="flex flex-col gap-2 text-sm text-muted">
              <div className="flex items-start gap-2">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="mt-0.5 flex-shrink-0 text-brand">
                  <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z" />
                  <circle cx="12" cy="10" r="3" />
                </svg>
      ...
      <div className="border-t border-line-soft">
        <div className="max-w-[1440px] mx-auto px-6 sm:px-10 lg:px-16 py-5 flex flex-col sm:flex-row items-center justify-between gap-3 text-sm text-muted">
          <p>&copy; {year} Nuansa Rinjani Rental. {f.rights}</p>
          <div className="flex gap-5">
            <Link href="/" className="hover:text-brand transition-colors">{f.privacy}</Link>
            <Link href="/" className="hover:text-brand transition-colors">{f.terms}</Link>
          </div>
        </div>
      </div>
    </footer>
